function initializeCarousel(sectionElement, imagePathPrefix, imageExtension, Max, num){
    setTimeout(getpre,1000);
    timer=setInterval(getpre,6000);
    var pics=new Array();
    var dots=new Array();
    var banner = sectionElement.querySelector('.banner');
    for(var i=1;i<=num;i++){
        var lbli=document.createElement('li');
        var lbimg=document.createElement('img');

        lbimg.src = imagePathPrefix+i+imageExtension;
        lbimg.style.maxWidth= Max;
        lbimg.style.maxHeight= Max;
...
function getpre(){
    ...
    pics[len-2].style.left="-200px";            ////左
    pics[len-2].style.opacity=0.5;
    pics[len-3].style.opacity=0;
    pics[len-4].style.opacity=0;
    pics[len-1].style.zIndex=100;
    pics[len-1].style.left="200px";             ////中
    pics[len-1].style.transform="scale(1.4)";
    pics[len-1].style.opacity=1;
    pics[len].style.left="600px";               ////右
    pics[len].style.opacity=0.5;
    dotmov();
    pics[len-2].onclick=function(){
        getnext();
